...
from shared.runner import ModelHandle, load_model  # noqa: E402
...
def _confirm_long_run(items: list[dict], sampling: SamplingCfg, max_tokens: int, prompt_id: str) -> None:
    # Rough estimate: items × n_samples × (max_tokens / 50 tok/s for HF, ~500 tok/s vLLM).
    # We don't know the backend yet at config time, so we use the conservative number.
    ...
    print(
        ...
        f"Set eval.confirm_long_runs=true to skip this prompt, or rerun with a smaller slice."
    )
...
def _score_and_save(
    ...
    subdir = "full_eval" if is_full_eval else ""
    out_path = results_dir / subdir / f"{run_id}.jsonl"
...
def main(cfg: DictConfig) -> None:
    ...
    model_cfg: ModelCfg = _typed(cfg.model, ModelCfg)  # type: ignore[assignment]
    ...
    print(f"Loading model: {model_cfg.model_id} (backend={model_cfg.backend})")
    handle = load_model(model_cfg)
    print(f"Model loaded: backend={handle.backend}")

    leaderboard_rows: list[dict] = []
    is_full_eval = eval_cfg.slice == "full"
    for entry in runs:
        prompt, sampling, max_tokens = _resolve_entry(entry, default_regime, eval_cfg.max_tokens)
        ...
        leaderboard = _score_and_save(
            prompt, items, gen_rows, sampling, max_tokens, handle, results_dir, is_full_eval
        )
        leaderboard["wall_time_s"] = elapsed
