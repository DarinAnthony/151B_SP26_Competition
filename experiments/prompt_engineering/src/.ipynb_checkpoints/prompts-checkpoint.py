...
COT_EXPLICIT = Prompt(
    ...
    notes="Explicit step-per-line CoT with an early-commit instruction.",
)
...
COT_STRUCTURED = Prompt(
    ...
    notes="Forces a fixed structure; intended to bound the Compute stage so the model doesn't ramble.",
)
...
FEWSHOT_2_BASIC = Prompt(
    ...
    notes="2 worked exemplars per modality. Terse reasoning + boxed answer style.",
)
...
FEWSHOT_3_DIVERSE = Prompt(
    ...
    notes="3 exemplars per modality spanning algebra, calculus, combinatorics.",
)
...
SC_DIVERSE_PATHS = Prompt(
    ...
    notes="Designed to pair with high-temperature N-sample sampling. Voting handled by shared.voting.",
)
...
SC_TERSE = Prompt(
    ...
    notes="Short responses keep N-sample voting cheap.",
)
...
PHP_BASIC = Prompt(
    ...
    notes="Plain PHP. Iteration handled by shared.multi_turn.run_php.",
)
...
PHP_SKEPTICAL = Prompt(
    ...
    notes="Skepticism cue to fight answer-anchoring bias.",
)
...
PLAN_AND_SOLVE = Prompt(
    ...
    notes="Wang et al. 2023 Plan-and-Solve baseline.",
)

SELF_REFINE_1PASS = Prompt(
    id="self_refine_1pass",
    category="other-self-refine",
    kind="self_refine",
    system_free=(
    ...
    notes="Two-turn self-critique. Iteration handled by shared.multi_turn.run_self_refine.",
)
