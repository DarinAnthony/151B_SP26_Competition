...
def majority_vote(extracted_list: list[str]) -> str:
    ...
    for s in non_empty:
        if counts[s] == top_count:
            return s
    raise AssertionError("unreachable: top_count is by construction the count of some element of non_empty")
...
def vote_index(extracted_list: list[str]) -> int:
    ...
    for i, s in enumerate(extracted_list):
        if s == winner:
            return i
    raise AssertionError("unreachable: winner is drawn from extracted_list")
