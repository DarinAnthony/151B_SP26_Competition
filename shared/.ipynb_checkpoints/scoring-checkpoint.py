...
import sympy
from sympy import I, Rational, S, oo, simplify, sympify
...
def _score_free(item: dict, response: str) -> ScoredResult:
    ...
    if not extracted:
        return ScoredResult(False, "", finished, None)
    ...
    try:
        if len(gold_list) == 1:
            ...
            if len(pred_parts) != len(gold_list):
                return ScoredResult(False, extracted, finished, None)
            correct = all(
                _free_form_match(p, str(g)) for p, g in zip(pred_parts, gold_list)
            )
    except Exception as e:
        return ScoredResult(False, extracted, finished, str(e))
...
# Backwards-compatible exports kept for `multi_turn.py` etc.
_extract_boxed_content = _last_boxed_content
